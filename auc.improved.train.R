...
fold.possible <- unique(folds.dt[, .(set.name, test.fold=fold)])
...
for(i in seq_along(i.todo)){
  ...
  train.fold.errors <- errors.dt[
    test.fold.info, on=.(set.name)][fold != test.fold]
  train.fold.errors[, min.log.lambda := min.log.penalty]
  train.fold.errors[, max.log.lambda := max.log.penalty]
  train.fold.errors[, seg.i := cumsum(
    c(1, diff(fp)!=0 | diff(fn) != 0)), by=.(prob.dir)]
  possible.errors <- possible.dt[train.fold.errors, on=list(
    set.name, fold, prob.dir)]
  possible.errors[, possible.fn := possible.tp]
  train.fold.segs <- possible.errors[, .(
    min.log.lambda=min(min.log.lambda),
    max.log.lambda=max(max.log.lambda)
  ), by=.(prob.dir, seg.i, fp, fn, possible.fn, possible.fp)]
  train.fold.segs[, wfp := fp*possible.fn]
  train.fold.segs[, wfn := fn*possible.fp]
  train.fold.segs[, errors := wfp+wfn]#equal weight errors.
  train.fold.segs[, mid.log.lambda := (max.log.lambda+min.log.lambda)/2]
  pred.list <- list()
  ## only need to compute once:
  thresh.dt <- train.fold.errors[order(-min.log.lambda), {
    fp.diff <- diff(fp)
    fn.diff <- diff(fn)
  ...
  ## initialization:
  first.dt <- train.fold.errors[max.log.lambda==Inf]
  target.list <- list(
    label=train.fold.errors,
    class=train.fold.segs)
...
(auc.improved.train <- do.call(rbind, auc.improved.list))

all.pred.dt <- auc.improved.train[, pred[[1]], by=list(
  test.fold.i, test.fold, pred.name)]
all.pred.dt[, table(test.fold.i, pred.name)]
all.pred.dt[, .(folds=.N), by=.(prob.dir, pred.name)][, .(
  min.folds=min(folds),
  max.folds=max(folds)
), by=.(set=sub("/.*", "", prob.dir), pred.name)]
pred.wide <- dcast(
  all.pred.dt,
  prob.dir + test.fold ~ pred.name,
  value.var="pred.log.lambda")
fwrite(pred.wide, "auc.improved.train.csv")

saveRDS(auc.improved.train, "auc.improved.train.rds")
